...
# Download the following package, if you don't already have it: 
#install.packages("alfa_0.0-0.tar.gz", repos=NULL, type="source")
...
# Pull and count total number of downloads per platform
	platTots	<- data.frame(
					"Category"= 
						c(	"Linux/Deb:","Linux/Rpm:","Linux/Tar:",
							"Linux/Solaris:","Mac OS:","Windows:"),		
					"Total"= 
						c(	length(grep(".deb",smLog[,"Request"])),
...
# Pull and count running download total and total unique IP's and HTTP codes	
	runUnique	<- data.frame(	
					"Category"= 
						c(	"Running Total:","Unique IP's:","Unique HTTP's:"),
					"Total"= 
						c( 	sum(platTots[,"Total"]),
